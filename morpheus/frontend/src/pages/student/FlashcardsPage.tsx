...
const flashcardData: Record<string, { q: string; a: string }[]> = {
    "permutations-combinations": [
        ...
        { q: "What is 0!?", a: "0! = 1\n\nThis is a convention, not calculated. It ensures formulas like nPn = n! and nCn = 1 work correctly." },
    ...
    "electrochemistry": [
        { q: "What is the electrochemical series?", a: "A list of elements arranged in order of their standard electrode potentials (E°) from most negative to most positive.\n\nElements higher up are stronger reducing agents; lower = stronger oxidising agents." },
        { q: "State the Nernst equation.", a: "E = E° - (RT/nF) × ln(Q)\n\nAt 25°C: E = E° - (0.0592/n) × log(Q)\n\nWhere n = moles of electrons, Q = reaction quotient" },
        { q: "What is the difference between a galvanic and electrolytic cell?", a: "Galvanic Cell: Converts chemical energy → electrical energy. Reaction is spontaneous. (e.g., battery)\n\nElectrolytic Cell: Converts electrical energy → chemical energy. Reaction is non-spontaneous. (e.g., electroplating)" },
        { q: "What is the standard hydrogen electrode (SHE)?", a: "Reference electrode with E° = 0.00 V by convention.\n\nConditions: H⁺ at 1M concentration, H₂ gas at 1 atm, 25°C.\n\nAll standard electrode potentials are measured against SHE." },
    ...
    "rotational-motion": [
        { q: "What is moment of inertia (I)?", a: "I = Σ mᵢrᵢ²\n\nThe rotational analog of mass. It depends on both mass and how it's distributed relative to the rotation axis.\n\nUnit: kg·m²" },
        { q: "State the parallel axis theorem.", a: "I = I_cm + Md²\n\nThe moment of inertia about any axis = moment of inertia about a parallel axis through the centre of mass + (total mass × square of distance between the axes)" },
        { q: "What is torque (τ) and its formula?", a: "Torque is the rotational equivalent of force.\n\nτ = r × F = rF sin(θ)\n\nUnit: N·m\n\nτ = Iα (rotational analog of F = ma)" },
        { q: "What is angular momentum (L)?", a: "L = Iω\n\nWhere I = moment of inertia, ω = angular velocity.\n\nConservation of angular momentum: If no external torque acts, L remains constant.\n\nL = r × p (for a point mass)" },
        { q: "What is the moment of inertia of a solid sphere about its diameter?", a: "I = (2/5)MR²\n\nFor a hollow sphere: I = (2/3)MR²\n\nFor a disc about its axis: I = (1/2)MR²\n\nFor a ring about its axis: I = MR²" },
    ],
    "probability": [
        { q: "State the addition theorem of probability.", a: "P(A ∪ B) = P(A) + P(B) - P(A ∩ B)\n\nFor mutually exclusive events: P(A ∪ B) = P(A) + P(B)" },
        { q: "What is Bayes' theorem?", a: "P(A|B) = P(B|A) × P(A) / P(B)\n\nIt updates the probability of event A given that B has already occurred." },
    ...
    "integration-by-parts": [
        ...
        { q: "What is the LIATE rule?", a: "LIATE gives priority order for choosing u in integration by parts:\n\nL: Logarithms (ln x)\nI: Inverse trig (arctan x, arcsin x)\nA: Algebraic (x², x, xⁿ)\nT: Trigonometric (sin x, cos x)\nE: Exponential (eˣ, aˣ)\n\nChoose the function that appears earlier in LIATE as u." },
        { q: "Evaluate ∫x·eˣ dx", a: "Let u = x → du = dx\nLet dv = eˣ dx → v = eˣ\n\n∫x·eˣ dx = x·eˣ - ∫eˣ dx = x·eˣ - eˣ + C = eˣ(x - 1) + C" },
    ],
};

const DEFAULT_CARDS = [
    { q: "What is the core concept of this topic?", a: "This topic hasn't been fully loaded yet. Review your textbook or ask your tutor for a detailed breakdown of the fundamentals." },
    { q: "How does this relate to other subjects?", a: "Many concepts in this topic connect to broader themes. Look for patterns and try to link this to what you already know." },
    { q: "What are the most common exam mistakes?", a: "Students often rush through calculations, skip steps, or confuse similar formulas. Write out every step clearly during practice." },
];
    ...
    return (
        ...
        >
            ...
            >
                {/* Front */}
                <div
                    className="absolute inset-0 rounded-2xl border border-morpheus-border bg-gradient-to-br from-morpheus-surface to-morpheus-bg p-7 flex flex-col items-center justify-center"
                    style={{ backfaceVisibility: "hidden" }}
                >
                    <span className="text-xs text-morpheus-muted uppercase tracking-widest mb-5 font-semibold">Question</span>
                    <p className="font-display text-lg sm:text-xl font-semibold text-morpheus-text text-center leading-relaxed">
                        {front}
                    </p>
                    <p className="mt-6 text-xs text-morpheus-muted flex items-center gap-1.5">
                ...
                {/* Back */}
                <div
                    className="absolute inset-0 rounded-2xl border border-morpheus-accent/30 bg-gradient-to-br from-morpheus-accent/10 via-morpheus-surface to-morpheus-bg p-7 flex flex-col items-start justify-center"
                    style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
                >
                    <span className="text-xs text-morpheus-accent uppercase tracking-widest mb-4 font-semibold">Answer</span>
                    <p className="text-sm sm:text-base text-morpheus-text leading-relaxed whitespace-pre-line">
                        {back}
                    </p>
                </div>
            </div>
    ...
    if (sessionDone) {
        ...
        return (
            <StudentLayout>
                <button
                    onClick={() => navigate(-1)}
                    className="flex items-center gap-2 text-sm text-morpheus-muted hover:text-morpheus-text mb-6"
                >
                ...
                <div className="max-w-md mx-auto text-center">
                    ...
                    <p className="text-morpheus-muted text-sm mb-6">You've reviewed all {cards.length} cards for <span className="text-morpheus-text font-medium">{topicName}</span></p>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-8">
                        ...
                        <div className="rounded-2xl border border-red-500/30 bg-red-500/5 p-4">
                            ...
                            <p className="text-xs text-morpheus-muted">Needs more work</p>
                        </div>
                    </div>

                    {unknownCount > 0 && (
                        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 mb-6 text-left">
                            <p className="text-sm text-amber-400 font-medium mb-1">💡 Tip</p>
                            <p className="text-xs text-morpheus-muted">
                                You still have {unknownCount} card{unknownCount > 1 ? "s" : ""} to master. Consider booking a tutor session or reviewing the cards again.
                            </p>
                        </div>
                    )}
        ...
        <StudentLayout>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                <div>
                    ...
                    <p className="text-xs text-morpheus-muted mt-0.5">{cards.length} flashcards · Tap card to flip</p>
                </div>
            ...
            <div className="flex gap-3 mb-6">
                ...
                <div className="flex items-center gap-1.5 text-xs text-red-400">
                    <XCircle size={13} />
                    <span>{unknownCount} to review</span>
                </div>
                <div className="flex items-center gap-1.5 text-xs text-morpheus-muted ml-auto">
                    <Zap size={13} />
                    <span>{cards.length - currentIndex - 1} left</span>
                </div>
            </div>
            ...
            <div className="max-w-2xl mx-auto">
                <Flashcard
                    front={cards[currentIndex].q}
                    back={cards[currentIndex].a}
                    flipped={flipped}
